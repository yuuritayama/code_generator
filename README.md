...
```
16進数（0-9, a-f のみ）で生成しますか？ (y/n): y
コードの文字数を入力してください（例: 6）: 12
A-F の大文字で出力しますか？ (y/n): n
生成するコード数を入力してください（例: 20）: 15
```

上記のように y を選ぶと、16 進数（例：0df888f11f96 のような形式）でコードを生成します。
n を選んだ場合は、従来通りのモードになります：

```
16進数（0-9, a-f のみ）で生成しますか？ (y/n): n
